...
<div<#if id?has_content> id="${id}"<#if id=="app-navigation"><#--class="cssmenu"--> style="display:none"</#if><#elseif style?has_content> class="${style}"<#elseif name?has_content>name="${name}"</#if>>
...
<script>
<#--$("#app-navigation nav ul").unwrap("nav");
$("#app-navigation  ul").removeClass("fancyNav");
$("#app-navigation > ul > li > a").wrap("<span></span>");
-->
$('div.nav-tabs1').find("a").addClass('button1 facebook');
$('div.div-button').find("a").addClass('button1 facebook');
$('div.button-style-2').find("a").addClass('button1 facebook');
$('div.rectangle-list').find("a").removeClass('button1 facebook');
...
<#macro renderLink linkUrl parameterList targetWindow uniqueItemName actionUrl linkType="" id="" style="" name="" height="" width="" text="" imgStr="">
    ...
    <#else>
        ...
        <a  <#if id?has_content> id="${id}"</#if><#if name?has_content> name="${name}"</#if><#if targetWindow?has_content> target="${targetWindow}"</#if> href="<#if "hidden-form"==linkType>javascript:document.${uniqueItemName}.submit()<#else>${linkUrl}</#if>"><#rt/>
        </#if>
        <#if imgStr?has_content>${imgStr}</#if>${text} <#rt/>
