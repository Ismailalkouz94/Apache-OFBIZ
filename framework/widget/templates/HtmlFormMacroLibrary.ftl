...
<#macro renderTextareaField name className alert cols rows maxlength id readonly value visualEditorEnable buttons tabindex language="">
  <textarea class="form-control" name="${name}"<#t/>
    <@renderClass className alert />
...
<script>
if("${title}"=="Format: yyyy-MM-dd HH:mm:ss.SSS"||"${title}"=="النمط: ي ي-ش ش-ع ع ع ع  س س:د د:ث ث"){
...
$("#${id}_i18n").datepicker({
	...
	if(document.getElementById("${id}").className=="required"){
	document.getElementById("${id}_i18n").className=document.getElementById("${id}").className;
          }

$( "#${id}_i18n" ).change(function() {
 document.getElementById("${id}").value = document.getElementById("${id}_i18n").value;
});
</script>
    <#if dateType!="time" >
    ...
    <#if timeDropdown?has_content && timeDropdown=="time-dropdown">
      <select class="form-control" style="width:80px" name="${timeHourName}" <#if classString?has_content>class="${classString}"</#if>><#rt/>
        <#if isTwelveHour>
          <#assign x=11>
          <#list 0..x as i>
        ...
        </select>:<select style="width:80px" class="form-control" name="${timeMinutesName}" <#if classString?has_content></#if>><#rt/>
        ...
        <#rt/>
        <#if isTwelveHour>
          <select class="form-control" name="${ampmName}" ><#rt/>
...
<div  style="margin-top:4px;margin-bottom:4px;display: -webkit-inline-box;">
 <#list items as item>
    <label class="checkbox-inline"><#rt/>
      <input class="checkbox style-0" type="checkbox"<#if (item_index == 0)> id="${id}"</#if><#rt/><#if tabindex?has_content> tabindex="${tabindex}"</#if><#rt/>
        <#if allChecked?has_content && allChecked> checked="checked" <#elseif allChecked?has_content && !allChecked>
          ...
          <span style="margin-left: 0.8px;"> ${item.description?default("")}</span></label>
  </#list>
</div>
</#macro>
...
<label class="radio radio-inline">															
    ...
    <span <@renderClass className alert />><#rt/>
      <input class="radiobox" type="radio"<#if currentValue?has_content><#if currentValue==item.key> checked="checked"</#if><#if tabindex?has_content> tabindex="${tabindex}"</#if><#rt/>
...
<#macro renderSubmitField buttonType className alert formName title name event action imgSrc confirmation containerId ajaxUrl tabindex>
  ...
  <#else>
    ...
    <input class="btn btn-primary1"  type="<#if containerId?has_content>button<#else>submit</#if>" 
...
<#macro renderFieldTitle style title id fieldHelpText="" for="">
  <label <#if for?has_content>for="${for}"</#if> <#if fieldHelpText?has_content> title="${fieldHelpText}"</#if><#if id?has_content> id="${id}"</#if>><#t/>
    ${title}<#t/>
  </label><#t/>
</#macro>
...
<#macro renderFormOpen linkUrl formType targetWindow containerId containerStyle autocomplete name viewIndexField viewSizeField viewIndex viewSize useRowSubmit>
 ...
 <form style="width:100%" class="smart-form" method="post" action="${linkUrl}"<#if formType=="upload"> enctype="multipart/form-data"</#if><#if targetWindow?has_content> target="${targetWindow}"</#if><#if containerId?has_content> id="${containerId}"</#if> onsubmit="javascript:submitFormDisableSubmits(this)"<#if autocomplete?has_content> autocomplete="${autocomplete}"</#if> name="${name}"><#lt/>
...
<#macro renderFormatSingleWrapperOpen formName style>
  <table cellspacing="0" <#if style?has_content>class="${style}"</#if> width="100%">
</#macro>
...
<#macro renderTextFindField name value defaultOption opEquals opBeginsWith opContains opIsEmpty opNotEqual className alert size maxlength autocomplete titleStyle hideIgnoreCase ignCase ignoreCase tabindex>
  <#if opEquals?has_content>
    ...
    <select <#if name?has_content>name="${name}_op"</#if> class="form-control"><#rt/>
  ...
  <#else>
    ...
    <input  class="form-control" type="hidden" name=<#if name?has_content> "${name}_op"</#if>    value="${defaultOption}"/><#rt/>
...
<#if titleStyle?has_content><span class="${titleStyle}" ><#rt/></#if>
    <#if hideIgnoreCase>
      ...
      <input  class="form-control" type="hidden" name="${name}_ic" value=<#if ignCase>"Y"<#else> ""</#if>/><#rt/>

<#else>
      <div style="display:none"><input  type="checkbox" name="${name}_ic" value="Y" <#if ignCase> checked="checked"</#if> /> ${ignoreCase}<#rt/></div>
...
<fieldset>
         <label class="input"> <i class="icon-append fa fa-calendar"></i>
            <input style="height: 28px !important" id="${name?html}_fld0_value" <@renderClass className alert />  type="text" <#if name?has_content> name="${name?html}_fld0_value"</#if><#if localizedInputTitle?has_content> title="${localizedInputTitle}"</#if><#if value?has_content> value="${value}"</#if><#if size?has_content> size="${size}"</#if><#if maxlength?has_content> maxlength="${maxlength}"</#if>/><#if tabindex?has_content> tabindex="${tabindex}"</#if><#rt/>
           </label>	
</fieldset>
<script>
if("${localizedInputTitle?html}"=="Format: yyyy-MM-dd"||"${localizedInputTitle?html}"=="النمط: ي ي-ش ش-ع ع ع ع"){
...
<#if dateType != "time">
    ...
    <select class="form-control"  <#if name?has_content> name="${name}_fld0_op"</#if> ><#rt/>
...
<fieldset>
         <label class="input"> <i class="icon-append fa fa-calendar"></i>
    <input  style="height: 28px !important;margin-left:2px" id="${name?html}_fld1_value" type="text" <@renderClass className alert /><#if name?has_content> name="${name}_fld1_value"</#if><#if localizedInputTitle??> title="${localizedInputTitle?html}"</#if><#if value2?has_content> value="${value2}"</#if><#if size?has_content> size="${size}"</#if><#if maxlength?has_content> maxlength="${maxlength}"</#if>/><#rt/>
   </label>	
</fieldset>
<script>
if("${localizedInputTitle?html}"=="Format: yyyy-MM-dd"||"${localizedInputTitle?html}"=="النمط: ي ي-ش ش-ع ع ع ع"){
...
$('#${name?html}_fld1_value').datetimepicker({
 ...
 <#if dateType != "time">
    ...
    <select  name=<#if name?has_content>"${name}_fld1_op"</#if> class="form-control"><#rt/>
...
<#macro renderRangeFindField className alert name value size maxlength autocomplete titleStyle defaultOptionFrom opEquals opGreaterThan opGreaterThanEquals opLessThan opLessThanEquals value2 defaultOptionThru tabindex>
 ...
 <input  <#--style="float:left;margin-right:5px"-->  type="text" <@renderClass className alert /> <#if name?has_content>name="${name}_fld0_value"</#if><#if value?has_content> value="${value}"</#if><#if size?has_content> size="${size}"</#if><#if maxlength?has_content> maxlength="${maxlength}"</#if><#if autocomplete?has_content> autocomplete="off"</#if>/><#if tabindex?has_content> tabindex="${tabindex}"</#if><#rt/>

 <#if titleStyle?has_content>
    <span class="${titleStyle}" ><#rt/>
  </#if>

  <select class="form-control" style="float:left;margin-right:5px"  <#if name?has_content>name="${name}_fld0_op"</#if> class="selectBox"><#rt/>
    <option value="equals"<#if defaultOptionFrom=="equals"> selected="selected"</#if>>${opEquals}</option><#rt/>
    <option value="greaterThan"<#if defaultOptionFrom=="greaterThan"> selected="selected"</#if>>${opGreaterThan}</option><#rt/>
...
<#rt/>
 ...
 <input style="float:left;margin-right:5px" type="text" <@renderClass className alert /><#if name?has_content> name="${name}_fld1_value"</#if><#if value2?has_content> value="${value2}"</#if><#if size?has_content> size="${size}"</#if><#if maxlength?has_content> maxlength="${maxlength}"</#if><#if autocomplete?has_content> autocomplete="off"</#if>/><#rt/>

 <#if titleStyle?has_content>
    <span class="${titleStyle}" ><#rt/>
  </#if>

  <select style="float:left;margin-right:5px"  class="form-control" name=<#if name?has_content>"${name}_fld1_op"</#if> class="selectBox"><#rt/>
...
<span class="field-lookup">
    <#if size?has_content && size=="0">
      <input class="form-control" type="hidden" <#if name?has_content> name="${name}"/></#if><#if tabindex?has_content> tabindex="${tabindex}"</#if><#rt/>
    <#else>

<fieldset>
<label class="input"> 
<span id="spanme"><a  id="_lookupId_button" class="btn btn-default" href="javascript:void(0);">...</a></span>
  ...
  <#if listSize gt viewSize>
    <div class="${paginateStyle}">
      <ul class="pagination dark-theme simple-pagination">
        ...
        <#if listSize gt 0 && javaScriptEnabled><li class="nav-page-select">&ensp;<label>${pageLabel}</label>&ensp;<select style="width:60px" class="form-control" name="page" size="1" onchange="<#if ajaxEnabled>ajaxUpdateAreas('${ajaxSelectUrl}')<#else>submitPagination(this, '${selectUrl}'+this.value)</#if>"><#rt/>
        ...
        <#if javaScriptEnabled><li class="nav-pagesize"><select style="width:61px" class="form-control" name="pageSize" size="1" onchange="<#if ajaxEnabled>ajaxUpdateAreas('${ajaxSelectSizeUrl}')<#else>submitPagination(this, '${selectSizeUrl}')</#if>"><#rt/>
          ...
          </select> <label>${paginateViewSizeLabel}</label></li>
        </#if>
        <li class="nav-displaying"> &ensp;<label style="font-weight:bold">${commonDisplaying}</label></li>
      </ul>
    </div>
  </#if>
</#macro>
...
<#macro renderFileField className alert name value size maxlength autocomplete tabindex>
  <input type="file" class="btn btn-default" <@renderClass className alert /><#if name?has_content> name="${name}"</#if><#if value?has_content> value="${value}"</#if><#if size?has_content> size="${size}"</#if><#if maxlength?has_content> maxlength="${maxlength}"</#if><#if autocomplete?has_content> autocomplete="off"</#if>/><#if tabindex?has_content> tabindex="${tabindex}"</#if><#rt/>
</#macro>
...
<div class="fieldgroup<#if style?has_content> ${style}</#if>"<#if id?has_content> id="${id}"</#if>>
...
<#macro renderHyperlinkTitle name title showSelectAll="N">
  <#if title?has_content><label class="checkbox">${title}  </#if>
  <#if showSelectAll="Y"><input type="checkbox" name="selectAll" value="Y" onclick="javascript:toggleAll(this, '${name}');"/><i></i></label></#if>
</#macro>
...
<#macro renderTooltip tooltip tooltipStyle>
  <#if tooltip?has_content><span class="<#if tooltipStyle?has_content>${tooltipStyle}<#else>tooltip1</#if>">${tooltip}</span><#rt/></#if>
</#macro>
...
<#macro makeHyperlinkString linkStyle hiddenFormName event action imgSrc title targetParameters alternate linkUrl targetWindow description confirmation uniqueItemName="" height="" width="" id="">
    ...
    <a <#if linkStyle?has_content && (description?has_content || imgSrc?has_content)>class="${linkStyle}"</#if>  
      ...
      <#if "${description}"=="PDF"><img src="/images/BusinessInnovation/images/iconsmart/icon 3d-04.png" alt="PDF" title="PDF"/>

      <#else>${description}</#if></a>
    </#if>
</#macro>
