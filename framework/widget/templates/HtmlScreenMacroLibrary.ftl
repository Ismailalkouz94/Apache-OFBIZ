...
<#macro renderScreenEnd>
    <script src="/partymgr/static/AccGLookup.js"></script>
</body>
</html>
...
<#macro renderLabel text id style>
  <#if text?has_content>
    ...
    <#if style?has_content>
      <#if style=="h1">
        <h1${idText} style="display:none" id ="h2contenttext">${text}</h1>
      <#elseif style=="h2">
       <h2 style="display:none" id ="h2contenttext" ${idText} >${text}</h2>
      <#elseif style=="h3">
        <h3${idText}>${text}</h3>
      <#elseif style=="h4">
        <h4${idText}>${text}</h4>
      ...
      <#elseif style=="h6">
        <h6${idText}>${text}</h6>
      <#else>
      <div role="content">  <span${idText} class="${style}">${text}</span></div>
      </#if>
    <#else>

      <h2 style="display:none" id ="h2contenttext">${text}</h2>
...
<#macro renderLink parameterList targetWindow target uniqueItemName linkType actionUrl id style name height width linkUrl text imgStr>
    <#if "layered-modal" != linkType>
     ...
     <div class="div-button">
      <li>
        <a class="button1 facebook" 
            <#if id?has_content>id="${id}"</#if> 
            <#if style?has_content>class="button1 facebook"</#if> 
            ...
            <#if imgStr?has_content>${imgStr}</#if>${text}&ensp;</a>
       </li></div>
    <#else>
...
<#if "${id}"=="LookupBulkAddProducts"||"${id}"=="ViewShoppingLists">
<#else>
<div class="row">
</#if>
<article class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
<div class="jarviswidget jarviswidget-color-blueDark" id="wid-id-" data-widget-editbutton="false">
<header>
...
<#macro renderScreenletEnd></div></div></div></article></div></#macro>
...
<#macro renderPortalPageColumnBegin originalPortalPageId portalPageId columnSeqId confMode="false" width="auto" delColumnLabel="Delete column" delColumnHint="Delete this column" addPortletLabel="Add portlet" addPortletHint="Add a new portlet to this column" colWidthLabel="Col. width:" setColumnSizeHint="Set column size">
  ...
  <div class="portal-column<#if confMode == "true">-config</#if>"  id="portalColumn_${columnSeqId}">
