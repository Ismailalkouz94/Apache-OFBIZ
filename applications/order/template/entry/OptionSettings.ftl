...
<form method="post" class="smart-form" action="<@ofbizUrl>finalizeOrder</@ofbizUrl>" name="checkoutsetupform">
        ...
        <tr>
          <td>
            <table width="100%" cellpadding="1" border="0" cellpadding="0" cellspacing="0">
              <tr>
                <td>
                  <h4>${uiLabelMap.OrderInternalNote}</h4>
                </td>
                <td>
                  <h4>${uiLabelMap.OrderShippingNotes}</h4>
                </td>
              </tr>
              <tr>
                <td>
...
<#assign supplier =  delegator.findOne("PartyGroup", Static["org.apache.ofbiz.base.util.UtilMisc"].toMap("partyId", supplierPartyId), false)! />
              ...
              <tr>
                <td>
                    <h2><b>${uiLabelMap.OrderShipGroup} ${uiLabelMap.CommonNbr} ${currIndex}</b><#if supplier?has_content> - ${supplier.groupName?default(supplier.partyId)}</#if></h2>
                </td>
              </tr>
               <#if cart.getOrderType() != "PURCHASE_ORDER">
                <tr>
                  <td>
                   <br/> <h4>${uiLabelMap.ProductShipmentMethod}</h4>
                  </td>
                </tr>
	            ...
	            <label style="margin:0" class="radio radio-inline">
                    <input type='radio' class="radiobox" name='${shipGroupIndex?default("0")}_shipping_method' value="Default" checked="checked" />
	            <span>${uiLabelMap.FacilityNoOtherShippingMethods}</span></label>
               ...
               <#else>
                   <tr>
                     <td>
                       <br/> <h2>${uiLabelMap.OrderOrderShipEstimate}</h2>
                     </td>
                   </tr>
                   <tr>
                ...
                <tr>
                  <td colspan='2'>
                   <br/>  <h2>${uiLabelMap.FacilityShipOnceOrAvailable}</h2>
                  </td>
                </tr>
                <tr>
                  <td valign="top">
	            <label style="margin:0" class="radio radio-inline">
                    <input type='radio' class="radiobox" <#if cart.getMaySplit(shipGroupIndex)?default("N") == "N">checked="checked"</#if> name='${shipGroupIndex?default("0")}_may_split' value='false' />
	            <span>${uiLabelMap.FacilityWaitEntireOrderReady}</span></label>
                  </td>
                </tr>
                <tr>
                  <td valign="top">
	           <label style="margin:0" class="radio radio-inline">
                    <input class="radiobox" <#if cart.getMaySplit(shipGroupIndex)?default("N") == "Y">checked="checked"</#if> type='radio' name='${shipGroupIndex?default("0")}_may_split' value='true' />
	          <span>${uiLabelMap.FacilityShipAvailable}</span></label>
                  </td>
                </tr>
                <tr>
                  <td colspan="2">
                    <br/> <h2>${uiLabelMap.OrderShipBeforeDate}</h2>
                  </td>
                </tr>
                <tr>
                    <td colspan="2">
                ...
                <tr>
                  <td colspan="2">
                    <br/> <h2>${uiLabelMap.OrderShipAfterDate}</h2>
                  </td>
                </tr>
                <tr>
                    <td colspan="2">
                ...
                <tr>
                  <td colspan="2">
                    <br/> <h2>${uiLabelMap.FacilitySpecialInstructions}</h2>
                  </td>
                </tr>
                <tr>
                  <td colspan="2">
                ...
                <#else>
				    ...
				    <label style="margin:0" class="radio">
				    <input class="radiobox" type="radio" <#if cart.getIsGift(shipGroupIndex)?default('Y') == 'Y'>checked="checked"</#if> name="${shipGroupIndex?default('0')}_is_gift" value="true" /><span>${uiLabelMap.CommonYes}</span>
				    </label>
				    <label style="margin:0" class="radio">
				    <input class="radiobox" type="radio" <#if cart.getIsGift(shipGroupIndex)?default('N') == 'N'>checked="checked"</#if> name="${shipGroupIndex?default('0')}_is_gift" value="false" /><span>${uiLabelMap.CommonNo}</span>
				    </label>
                                </div>
                            </td>
                        </tr>
                    ...
                    <tr>
                        <td colspan="2">
                            <br/> <h2>${uiLabelMap.OrderGiftMessage}</h2>
                        </td>
                    </tr>
                    <tr>
                        <td colspan="2">
