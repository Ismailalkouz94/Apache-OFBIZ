...
<table border="0" cellpadding="2" cellspacing="0" width="100%">
  ...
  <tr>
    ...
    <td valign="top">
      ...
      <label>${productContentWrapper.get("DESCRIPTION", "html")!}</label>
      ...
      <form method="post" action="<@ofbizUrl>additem<#if requestAttributes._CURRENT_VIEW_??>/${requestAttributes._CURRENT_VIEW_}</#if></@ofbizUrl>" name="addform" style='margin: 0;'>
        ...
        <#else>
          <#if inStock>
            ...
            <#else>
              <a href="javascript:addItem()" class="btn btn-primary"><span style="white-space: nowrap;">${uiLabelMap.OrderAddToCart}</span></a>&nbsp;
    ...
    <div>
      <#if sessionAttributes.userLogin?has_content && sessionAttributes.userLogin.userLoginId != "anonymous">
        ...
        <form name="addToShoppingList" method="post" action="<@ofbizUrl>addItemToShoppingList<#if requestAttributes._CURRENT_VIEW_??>/${requestAttributes._CURRENT_VIEW_}</#if></@ofbizUrl>">
          ...
          &nbsp;&nbsp;
          <input type="text" size="5" name="quantity" value="1" />
          <a href="javascript:document.addToShoppingList.submit();" class="btn btn-primary">[${uiLabelMap.OrderAddToShoppingList}]</a>
        </form>
      <#else> <br />
  ...
  <tr>
    <td colspan="2">
      <form name="configform" class="smart-form" id="configFormId" method="post" action="<@ofbizUrl>product<#if requestAttributes._CURRENT_VIEW_??>/${requestAttributes._CURRENT_VIEW_}</#if></@ofbizUrl>">
        <input type='hidden' name='add_product_id' value='${product.productId}' />
            ...
            <#else>
              <#if question.isSingleChoice()>
                ...
                <#if !question.isMandatory()>
                  <div><label class="radio radio-inline"><input class="radiobox" type="radio" name='${counter}' value='<#if !question.isSelected()>checked="checked"</#if>' /><span> No option</span></label></div>
                </#if>
                  ...
                  <#if option.isSelected()>
                    ...
                    <#if option.hasVirtualComponent()>
                      ...
                      <label class="radio radio-inline">
                        <input type='radio' class="radiobox" name='${counter}' id="${counter}_${optionCounter}" value='${optionCounter}' onclick="javascript:checkOptionVariants('${counter}_${optionCounter}');" />
                        <span>${option.description} <#if !option.isAvailable()> (*)</#if></span>
                      </label>
                    ...
                    <#else>
                      <div>
                      <label  class="radio radio-inline">
                        <input type="radio" class="radiobox" name='${counter}' value='${optionCounter}' <#if option.isSelected() || (!question.isSelected() && optionCounter == 0 && question.isMandatory())>checked="checked"</#if> />
                ...
                <#list options as option>
                    ...
                    <#if option.hasVirtualComponent()>
                      ...
                      <label class="checkbox-inline">
                        <input type='CHECKBOX' class="checkbox style-0" name='${counter}' id="${counter}_${optionCounter}" value='${optionCounter}' onclick="javascript:checkOptionVariants('${counter}_${optionCounter}');" />
                        <span>${option.description} <#if !option.isAvailable()> (*)</#if></span>
                      </label>
                        <#assign components = option.getComponents()>
                        <#list components as component>
                    ...
                    <div>
                    <label class="checkbox-inline">
                      <input type='CHECKBOX' class="checkbox style-0" name='${counter}' value='${optionCounter}' <#if option.isSelected()>checked="checked"</#if> />
                      <span>${option.description} +<@ofbizCurrency amount=option.price isoCode=price.currencyUsed/><#if !option.isAvailable()> (*)</#if></span>
                    </label>
                    </div>
                    </#if>
                    <div>${uiLabelMap.CommonComments}: <input type="text" name='comments_${counter}_${optionCounter}' id='comments_${counter}_${optionCounter}' value='${option.comments!}' /></div>
