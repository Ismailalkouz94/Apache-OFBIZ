...
<header>
<h2>${uiLabelMap.ProductVerify}&nbsp;${uiLabelMap.OrderOrder}&nbsp;${uiLabelMap.CommonIn}&nbsp;${facility.facilityName!} [${facility.facilityId!}]</h2>
</header>
<div role="content">
  <div class="screenlet">
    ...
    <#if shipmentId?has_content>
      <div>
        <span><label>${uiLabelMap.ProductShipmentId}</label></span><a href="<@ofbizUrl>/ViewShipment?shipmentId=${shipmentId}</@ofbizUrl>" class="buttontext">${shipmentId}</a>
      </div>
      <#if invoiceIds?? && invoiceIds?has_content>
        <div>
          <span><label>${uiLabelMap.AccountingInvoices}:</label></span>
          <ul>
            <#list invoiceIds as invoiceId>
              <li>
                ...
                (<a href="/accounting/control/invoice.pdf?invoiceId=${invoiceId}${StringUtil.wrapString(externalKeyParam)}" target="_blank" class="btn btn-primary1">PDF</a>)
              </li>
            </#list>
          </ul>
        </div>
    ...
    <div class="screenlet-body">
      <form name="selectOrderForm" method="post" action="<@ofbizUrl>VerifyPick</@ofbizUrl>">
        ...
        <table cellspacing="0" class="basic-table">
          <tr>
            <td><label>${uiLabelMap.ProductOrderId}</label></td>
            <td width="1">&nbsp;</td>
            <td>
              <#if shipmentId?has_content>
                <input class="form-control" type="text" name="orderId" size="20" maxlength="20" value=""/>
              <#else>
                <input class="form-control" type="text" name="orderId" size="20" maxlength="20" value="${orderId!}"/>
              </#if>
              /
              <input class="form-control" type="text" name="shipGroupSeqId" size="6" maxlength="6" value="${shipGroupSeqId?default("00001")}"/>
            </td>
            <td>&nbsp;</td>
          </tr>
          <tr>
            <td colspan="2">&nbsp;</td>
            <td colspan="2">
              <input type="submit" class="btn btn-primary" value="${uiLabelMap.ProductVerify}&nbsp;${uiLabelMap.OrderOrder}"/>
            </td>
          </tr>
        </table>
      </form>
      ...
      <form name="selectPicklistBinForm" method="post" action="<@ofbizUrl>VerifyPick</@ofbizUrl>" style="margin: 0;">
        ...
        <table cellspacing="0" class="basic-table">
          <tr>
            <td width="282px"><label>${uiLabelMap.FormFieldTitle_picklistBinId}</label></td>
            <td width="1">&nbsp;</td>
            <td>
              <input class="form-control" type="text" name="picklistBinId" size="29" maxlength="60" value="${picklistBinId!}"/>
            </td>
            <td>&nbsp;</td>
          </tr>
          <tr>
...
<div class="jarviswidget jarviswidget-color-blueDark" id="wid-id-0" data-widget-editbutton="false">
<header>
<h2>${uiLabelMap.ProductOrderId} ${uiLabelMap.CommonNbr}<a href="/ordermgr/control/orderview?orderId=${orderId}">${orderId}</a> / ${uiLabelMap.ProductOrderShipGroupId} #${shipGroupSeqId}</h2>
</header>
<div role="content">
    <div class="screenlet">
      <div class="screenlet-body">
...
<#else>
  <div class="alert alert-info fade in">${uiLabelMap.ProductFacilityViewPermissionError}</div>
</div>
</section>
</#if>
