...
<div>
          ...
          <tr>
            ...
            <td class="button-col">
              <#if security.hasEntityPermission("FACILITY", "_UPDATE", session)>
                <a class="btn btn-primary1" href='<@ofbizUrl>EditContactMech?facilityId=${facilityId}&amp;contactMechId=${contactMech.contactMechId}</@ofbizUrl>'>${uiLabelMap.CommonUpdate}</a>
             </#if>
              <#if security.hasEntityPermission("FACILITY", "_DELETE", session)>
                ...
                <br/><br/><br/> <a class="btn btn-primary1" href="javascript:document.deleteContactForm_${contactMechMap_index}.submit()">${uiLabelMap.CommonExpire}</a>
              </#if>
            </td>
          </tr>
      </#list>
  ...
  <#else>
    <div class="alert alert-info fade in">${uiLabelMap.CommonNoContactInformationOnFile}.</div>
  </#if>
</div>
