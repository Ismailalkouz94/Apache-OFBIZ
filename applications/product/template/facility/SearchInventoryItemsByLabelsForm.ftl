...
<form method="post" action="<@ofbizUrl>SearchInventoryItemsByLabels</@ofbizUrl>">
  <input class="form-control" type="hidden" name="facilityId" value="${facility.facilityId}"/>
  <table>
  <#assign index = 0>
  <#list labelTypes as labelType>
    ...
    <tr>
      <td>
          <div>
          <label>${labelType.description!} [${labelType.inventoryItemLabelTypeId}]</label>
          &nbsp;
          <select class="form-control" name="inventoryItemLabelId_${index}">
            <option></option>
  ...
  <input class="btn btn-primary1" type="hidden" name="numberOfFields" value="${index}"/>
</form>
