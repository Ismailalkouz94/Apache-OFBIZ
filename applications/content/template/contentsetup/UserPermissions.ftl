  ...
  <tr>
    <td width='100%'>
      ...
      <table width='100%' border='0' cellspacing='0' cellpadding='0' class='appTitle'>
        <tr>
          <td>
            <label>WebSitePublishPoint</label>
          </td>
          <td>

             <input class="form-control" type="text" name="webSitePublishPoint" size="20" value="${webSitePublishPoint!}" />
             <input class="btn btn-primary" type="submit" value="${uiLabelMap.CommonRefresh}"/>
             <input class="form-control" type="hidden" name="partyId" value="${partyId!}"/>
             <input class="form-control" type="hidden" name="userLoginId" value="${userLoginId!}"/>
            </div>
          </td>
        </tr>
      </table>
  ...
  <tr>
    <td width='100%'>
      ...
      <table width='100%' border='0' cellspacing='0' cellpadding='4' class='boxoutside'>
        <tr>
            <td><label>${uiLabelMap.ContentWebSite}</label></td>
      ...
      <#assign rowCount=0/>
        <#list siteList as map>
          <tr>
            <td><label>${map.partyId!}</label></td>
            <#list blogRoleIdList as roleTypeId>
              <#assign cappedSiteRole= Static["org.apache.ofbiz.entity.model.ModelUtil"].dbNameToVarName(roleTypeId) />
              <td >
              <input class="checkbox style-0" type="checkbox" name="${cappedSiteRole}_o_${rowCount}" value="Y" <#if map[cappedSiteRole]?has_content && map[cappedSiteRole] == "Y">checked="checked"</#if>/>
              </td>
            </#list>
          </tr>
          ...
          <tr>
            <td>
              <a class="btn btn-primary1" href="javascript:submitRows('${rowCount!}')">${uiLabelMap.CommonUpdate}</a>
            </td>
          </tr>
      </table>
      </form>
