...
<div class="screenlet screenlet-body">
    <#if !grandTotal??>
      <div>
        <span class="label" id="showFinAccountTransRunningTotal">${uiLabelMap.AccountingRunningTotal} :</span>
      </div>
    </#if>
      ...
      <#if (glReconciliationId?has_content && (glReconciliationId == "_NA_" && finAccountTransList?has_content)) || !grandTotal??>
        <div align="right">
          <#if grandTotal??>
            ...
            <#else>
              <span class="tooltip1">${uiLabelMap.AccountingNoGlReconciliatio??} <a href="<@ofbizUrl>EditFinAccountReconciliations?finAccountId=${parameters.finAccountId!}</@ofbizUrl>">${uiLabelMap.CommonClickHere}</a></span>
            </#if>
          <#else>
