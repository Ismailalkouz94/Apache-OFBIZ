...
<#if invoices?has_content >
    ...
    <label>${uiLabelMap.AccountingRunningTotalOutstanding} :</label>
    <label id="showInvoiceRunningTotal"></label>
  ...
  <form name="listPurchaseInvoices" id="listPurchaseInvoices"  method="post" action="javascript:void(0);">
    <div>
      ...
      <select class="form-control" name="serviceName" id="serviceName" onchange="javascript:setServiceName(this);">
      ...
      <input class="btn btn-primary" id="submitButton" type="button" onclick="javascript:runAction();" value="${uiLabelMap.CommonRun}" disabled="disabled" />
    </div>
    ...
    <div id="issueChecks" style="display: none;" align="right">
      <span><label>${uiLabelMap.AccountingVendorPaymentMethod}</label></span>
      ...
      <span><label>${uiLabelMap.AccountingCheckNumber}</label></span>
      <input class="form-control" type="text" name="checkStartNumber"/>
    </div>
    <table class="table table-bordered table-striped" cellspacing="0">
     ...
     <thead>
      <tr class="header-row-2">
        ...
        <th>${uiLabelMap.AccountingInvoiceDate}</th>
        <th>${uiLabelMap.AccountingDueDate}</th>
        <th>${uiLabelMap.CommonStatus}</th>
        <th>${uiLabelMap.AccountingReferenceNumber}</th>
        <th>${uiLabelMap.CommonDescription}</th>
        <th>${uiLabelMap.AccountingVendorParty}</th>
        <th>${uiLabelMap.AccountingToParty}</th>
        <th>${uiLabelMap.AccountingAmount}</th>
        <th>${uiLabelMap.FormFieldTitle_paidAmount}</th>
        <th>${uiLabelMap.FormFieldTitle_outstandingAmount}</th> 
        <th><label class="checkbox-inline"><input class="checkbox style-0" type="checkbox" id="checkAllInvoices" name="checkAllInvoices" onchange="javascript:toggleInvoiceId(this);"/><span>${uiLabelMap.CommonSelectAll}</span></label></th>
      </tr>
 </thead>
      <#-- Header Ends-->
      ...
      <#list invoices as invoice>
        ...
        <#assign invoicePaymentInfo = invoicePaymentInfoList.get("invoicePaymentInfoList").get(0)!>
          ...
          <tr valign="middle"<#if alt_row> class="alternate-row"</#if>>
            ...
            <td align="right"><label class="checkbox-inline"><input class="checkbox style-0" type="checkbox" id="invoiceId_${invoice_index}" name="invoiceIds" value="${invoice.invoiceId}" onclick="javascript:getInvoiceRunningTotal();"/><span></span></label></td>
          </tr>
...
<#else>
  <div class="alert alert-info fade in">${uiLabelMap.AccountingNoInvoicesFound}</div>
</#if>
