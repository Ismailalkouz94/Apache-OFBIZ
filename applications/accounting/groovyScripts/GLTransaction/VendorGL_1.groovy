...
List glAccountsParty = select("glAccountId").from("PartyGlAccount").where("roleTypeId", "BILL_FROM_VENDOR", "glAccountTypeId", "ACCOUNTS_PAYABLE","partyId",context.session.getAttribute("Lookup_partyId")).cache(true).queryList() 

List glAccountsDefault = select("glAccountId").from("GlAccountTypeDefault").where("glAccountTypeId", "ACCOUNTS_PAYABLE").cache(true).queryList()
