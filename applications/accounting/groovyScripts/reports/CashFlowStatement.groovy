...
if (!parameters.AccountClass) {
    parameters.AccountClass = "1"
}
...
GenericValue glAccountClass = null
if ("1" == parameters.AccountClass){
    if(null == parameters.glAccountClassId)
    glAccountClass = from("GlAccountClass").where("glAccountClassId", "CASH_EQUIVALENT").cache(true).queryOne()
    else
    glAccountClass = from("GlAccountClass").where("glAccountClassId",parameters.glAccountClassId).cache(true).queryOne() 
} else if ("2" == parameters.AccountClass){
    glAccountClass = from("GlAccountClass").where("glAccountClassId", "CASH_EQUIVALENT").cache(true).queryOne()
}
