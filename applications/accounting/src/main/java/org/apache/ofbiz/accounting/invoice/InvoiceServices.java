...
//    Ahmad Rbab'ah'
    public static String getNoOfDaysAP(HttpServletRequest request, HttpServletResponse response) throws GenericEntityException, IOException, ParseException, ParseException {
        System.out.println("Party Id = " + request.getParameter("partyId"));
        ...
        try {
            try {
                ...
                String sqlStr = "SELECT NO_OF_DAYS FROM VENDOR WHERE PARTY_ID ='" + request.getParameter("partyId") + "'";
                System.out.println("before sqlStr = " + sqlStr);
                resultSet = sqlProcessor.executeQuery(sqlStr);

                System.out.println("sqlStr = " + sqlStr);
                while (resultSet.next()) {
                    noOfDays = resultSet.getInt("NO_OF_DAYS");
                    break;
                }
...
//            Timestamp timestamp1 =Timestamp.valueOf(timestamp);
                ...
                Logger.getLogger(InvoiceServices.class.getName()).log(Level.SEVERE, null, ex);
            } catch (Exception ex) {
                Logger.getLogger(InvoiceServices.class.getName()).log(Level.SEVERE, null, ex);
            }
        } finally {
    ...
    public static String getNoOfDaysAR(HttpServletRequest request, HttpServletResponse response) throws GenericEntityException, IOException, ParseException, ParseException {
        System.out.println("Party Id = " + request.getParameter("partyId"));
        response.setCharacterEncoding("UTF-8");
        PrintWriter out = null;
        ...
        try {
            try {
                ...
                String sqlStr = "SELECT NO_OF_DAYS FROM PERSON WHERE PARTY_ID ='" + request.getParameter("partyId") + "'";
                System.out.println("before sqlStr = " + sqlStr);
                resultSet = sqlProcessor.executeQuery(sqlStr);
...
//            Timestamp timestamp1 =Timestamp.valueOf(timestamp);
    ...
    public static String getDateOfInvoice(HttpServletRequest request, HttpServletResponse response) throws GenericEntityException, IOException, ParseException, ParseException {
        System.out.println("**invoiceId Id = " + request.getParameter("invoiceId"));
        response.setCharacterEncoding("UTF-8");
        PrintWriter out = null;
        ...
        criteria.put("invoiceId", invoiceId);
        try {
            GenericValue resultInvoice = delegator.findOne("Invoice", criteria, true);
            String invoiceDate = String.valueOf(resultInvoice.get("invoiceDate"));
            System.out.println("invoiceDate " + invoiceDate);

            response.setContentType("text/html; charset=UTF-8");
            response.setCharacterEncoding("UTF-8");
            out = response.getWriter();
            out.println(invoiceDate);
            out.flush();
        } catch (Exception ex) {
            ex.getStackTrace();
        } finally {
